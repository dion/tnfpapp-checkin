    ...
    function all(){
    	...
    	if($result){
    	    ...
    	    for($i = 0; $i < count($clients); $i++){
        	        $query = "SELECT visit_items.* FROM visit_items INNER JOIN items ON visit_items.item = items.name AND items.place_of_service= :placeOfService WHERE c_id = :c_id and visit_items.timestamp <= ( NOW() - INTERVAL 7 DAY ) and active = 1  OR c_id = :c_id and visit_items.timestamp <= NOW() and active = 1";
    	   ...
    	   $query2 = "SELECT * FROM visit_items WHERE timestamp BETWEEN :startDate AND :endDate AND c_id = :c_id AND active = 1";
        	...
        	if($result2){
    	        ...
    	        $itemsArray = $stmt2->fetchAll();
    	                ...
    	                // $this->error = json_encode($itemsArray);
						// return false;
