...
try{
	...
	$stmt = $objDb->prepare("SELECT volunteer FROM volunteers WHERE active = 'Y' ORDER BY volunteer ASC");
	if(!$stmt->execute()){
		throw new PDOException('The result returned no object');
	}
	...
	// Querying visits separately
	$stmt = $objDb->prepare('SELECT id, place_of_service, date_of_visit as dateOfVisit, program, volunteer, numBags, weight, numOfItems, visitNotes FROM visits WHERE client_id = :id ORDER BY place_of_service, dateOfVisit DESC');
	if(!$stmt->execute(array('id' => $_POST['id']))){ 
